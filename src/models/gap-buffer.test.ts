...
test("initializes", () => {
  const gb = new GapBuffer<number | string>([]);
  expect(gb).toBeTruthy();
});

test("is iterable", () => {
  const gb = new GapBuffer<number | string>([1, 2, 3]);
  const arr = [];
  for (const x of gb) {
    arr.push(x);
  }
...
test("length is correct", () => {
  const gb = new GapBuffer<number | string>([1, 2, 3], 3);
...
test.each([...Array(100).keys()])(
  ...
  (buffSize) => {
    ...
    const gb = new GapBuffer<number | string>(arr, buffSize);
...
test("insert item at index 2 ", () => {
  const gb = new GapBuffer<number | string>([1, 2, 3, 4], 3);
  gb.insert(2, "hi");
  expect([...gb]).toEqual([1, 2, "hi", 3, 4]);
});
...
test.each([...Array(100).keys()])(
  ...
  (ix) => {
    const gb = new GapBuffer<number | string>([...Array(100).keys()]);
...
test("entries returns a iterable with index:value tuple", () => {
  const arr = [1, 2, 3, 4, 5];
  const gb = new GapBuffer<number | string>(arr, 3);
